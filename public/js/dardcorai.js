document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateModelUI(type) {
        ...
        const placeholderMap = { 'basic': 'Ask To Dardcor...', 'dark': 'Ask To Dardcor Dark...', 'pro': 'Ask Dardcor Pro...' };
        if (toolLabel) toolLabel.innerText = nameMap[type] || 'Basic Model';
        if (messageInput) messageInput.placeholder = placeholderMap[type] || 'Ask To Dardcor...';
    ...
    function applyMinimalistStyle() {
        ...
        for (let btn of allButtons) {
            if (btn.innerHTML.includes('Salin') || btn.innerHTML.includes('Dengar') || btn.querySelector('i')) {
    ...
    document.querySelectorAll('.model-select-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (modelType) setModel(modelType);
        });
    });
    ...
    function appendMessage(role, text, files = []) {
        ...
        if (text === '...loading_placeholder...') {
            ...
            div.innerHTML = `<div class="flex flex-col ${role === 'user' ? 'items-end' : 'items-start'} max-w-[85%] min-w-0">${fileHtml}<div class="chat-content-box relative rounded-2xl px-5 py-3.5 shadow-md text-sm ${bubbleClass} w-fit min-w-0 max-w-full overflow-hidden leading-7">${contentHtml}</div>${actionButtons}</div>`;
        }
    ...
    async function fetchPersonas() { try { const res = await fetch('/api/personas'); const data = await res.json(); const list = document.getElementById('persona-list'); if (list) { list.innerHTML = ''; if (data.success && data.data) { data.data.forEach(p => { const isActive = selectedPersonaId === p.id; const div = document.createElement('div'); div.className = `flex justify-between items-center p-2 rounded bg-[#151520] border ${isActive ? 'border-purple-900 ring-1 ring-purple-900' : 'border-white/10'} hover:border-purple-900/50 transition`; div.innerHTML = `<div class="cursor-pointer flex-1" onclick="selectPersona('${p.id}', '${p.name}', this)"><div class="font-bold text-xs text-white flex items-center gap-2">${p.name}${isActive ? '<i class="fas fa-check-circle text-purple-600"></i>' : ''}</div><div class="text-[10px] text-gray-400 truncate w-48">${p.instruction}</div></div><button onclick="window.requestToolDelete('Hapus persona ini?', async () => { try { const res = await fetch('/api/personas/${p.id}', { method: 'DELETE' }); if(res.ok) { if (selectedPersonaId === '${p.id}') deselectPersona(); window.showNavbarAlert('Persona dihapus', 'success'); fetchPersonas(); } else { window.showNavbarAlert('Gagal menghapus', 'error'); } } catch(e) { window.showNavbarAlert('Error sistem', 'error'); } })" class="text-red-400 hover:text-red-300 ml-2 p-1"><i class="fas fa-trash"></i></button>`; list.appendChild(div); }); } } } catch (e) { console.error(e); } }
    ...
    async function fetchVault() { try { const res = await fetch('/api/vault'); const data = await res.json(); const list = document.getElementById('vault-list'); if (list) { list.innerHTML = ''; if (data.success && data.data) { data.data.forEach(v => { list.innerHTML += `<div class="flex justify-between items-center p-2 rounded bg-[#151520] border border-white/10 mb-1"><div class="text-xs text-white font-bold truncate w-64">${v.title}</div><button onclick="window.requestToolDelete('Hapus dokumen ini?', async () => { try { const res = await fetch('/api/vault/${v.id}', { method: 'DELETE' }); if(res.ok) { window.showNavbarAlert('Dokumen dihapus', 'success'); fetchVault(); } else { window.showNavbarAlert('Gagal menghapus', 'error'); } } catch(e) { window.showNavbarAlert('Error sistem', 'error'); } })" class="text-red-400 hover:text-red-300"><i class="fas fa-trash"></i></button></div>`; }); } } } catch (e) { console.error(e); } }
    ...
    async function fetchMemories() { try { const res = await fetch('/api/memories'); const data = await res.json(); const list = document.getElementById('memory-list'); if (list) { list.innerHTML = ''; if (data.success && data.data) { data.data.forEach(m => { list.innerHTML += `<div class="flex justify-between items-center p-2 rounded bg-[#151520] border border-white/10 mb-1"><div class="text-xs text-white truncate w-64">${m.fact}</div><button onclick="window.requestToolDelete('Hapus memori ini?', async () => { try { const res = await fetch('/api/memories/${m.id}', { method: 'DELETE' }); if(res.ok) { window.showNavbarAlert('Memori dihapus', 'success'); fetchMemories(); } else { window.showNavbarAlert('Gagal menghapus', 'error'); } } catch(e) { window.showNavbarAlert('Error sistem', 'error'); } })" class="text-red-400 hover:text-red-300"><i class="fas fa-trash"></i></button></div>`; }); } } } catch (e) { console.error(e); } }
